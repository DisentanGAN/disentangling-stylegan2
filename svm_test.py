...
def calc_metrics(experiment, results, max_iter):
    ...
    """
    metrics = {"Path": experiment, "Mean": np.mean(results),"Median": np.median(results), "Std:": np.std(results), "Min": np.min(results),"Max": np.max(results), "SVM_Iterations":max_iter, "Raw_Data":results}
    return metrics 
...
for experiment in mnist_experiments:
    for c_val in c_values:
        perform_evaluation(experiment_path=experiment, dataset='mnist', max_iter=2000, c=c_val, save_to="svm-encoder-results-mnist")
        
# Conduct svm pcam experiment
for experiment in pcam_experiments:
    for c_val in c_values:
        perform_evaluation(experiment_path=experiment, dataset='pcam', max_iter=2000, c=c_val, save_to="svm-encoder-results-pcam")
